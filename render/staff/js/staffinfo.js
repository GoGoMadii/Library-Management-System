...
let ds = document.getElementById('Des')
...
function showStudent(res){
  ...
  rn.innerHTML=res[0].StaffID
  db.innerHTML=convertDate(res[0].DOB)
  ds.innerHTML=res[0].Designation
  m.innerHTML=res[0].PhoneNumber
  f.innerHTML=res[0].FINE

  // book Borrowed
  var sql ="SELECT * FROM `lms`.`book-transaction` where studentid="+res[0].StaffID+";"
  connection.query(sql,(err,res)=>{
    if(err) throw err;
    else{
      if (res.length==0) {
          console.log("No Book Lend");
      }else {
